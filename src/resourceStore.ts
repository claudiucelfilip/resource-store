...
export class ResourceStore {
  ...
  makeStore(resource: Resource) {
    let store = new Proxy(resource, {
      get: (target: Resource, name: string) => {
        ...
        return (
          this.get(compoundKey, true) ||
          this.makeStore(resource[symbol.select](name))
        ); //this.get(resource[symbol.select](name));
      }
    });
  ...
  get(keyResource: string | Resource, dontThrow = false) {
    let key: string = keyResource[symbol.key] || keyResource;
    let resource = this.stores[key];

    if (!resource && !dontThrow) {
      throw new Error(
        `No resource with key ${key[symbol.key] || key} was defined.`
      );
    }
